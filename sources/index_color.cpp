...
void Index_color::set_mmer_map(const mmer_map& mmermap){
    this-> mmermap = mmermap;
}



void Index_color::set_color_map(color_map* color_map){
    this-> colormap = color_map;
}



void Index_color::set_read_line_pos(const vector<uint64_t>& read_line_pos){
...
void Index_color::create_index_mmer_no_unique(const string& read_file, uint16_t k, uint16_t m, uint64_t min_occ, uint8_t counting_bf_size, bool homocomp, uint16_t num_thread){
    ...
    if(fichier){
        ...
        if(fichier_scd){
            ...
            robin_hood::unordered_map<color, icolor> map_current_read_color;
            string ligne; 
