                    ...
                    if (ligne.size() >= k) {
                        ...
                        for(uint im = 0; im < minimizer_list.size(); im++){
                            ...
                            if (mmermap.find(mmer) == mmermap.end()) {
                                ...
                                list_update_local[idmutex].push_back({previous_icolor, mmer});
                            }
                        }
                        #pragma omp barrier
                        #pragma omp for
                        ...
                        for(uint i = 0;i<1024;i++){
                            if(list_update[i].size() != 0){
                                for(auto elt : list_update[i]){
                                    cout << elt.first << " " << elt.second << endl;
                                }
                                cout << endl;
                                sort(list_update[i].begin(), list_update[i].end());
                                uint32_t icolor = list_update[i][0].first;
                                Color previous_color = colormap[i][icolor];
                                Color color_to_insert;
                                //uint32_t previous_icolor = list_update[i].first;
                                uint cpt = 1;
                                for(uint j = 1;j<list_update[i].size();j++){
                                    if(list_update[i][j].first != icolor){
                                        if(cpt == previous_color.get_nb_occ()){
                                            colormap[i][icolor].add_idread(global_num_read-1);
                                            cout << "if" << endl;
                                        }
                                        else{
                                            cout << "else " << cpt << "  " <<  previous_color.get_nb_occ() << endl;
                                            int new_id;
                                            color_to_insert = Color(previous_color, global_num_read - 1);
                                            #pragma omp critical (cic)
                                            {
                                                new_id = current_id_color*num_thread+omp_get_thread_num();
                                                current_id_color++;
                                            }
                                            colormap[i][new_id] = color_to_insert; // TO DO ICOLOR UINT64?
                                            for(uint ii = 0; ii<cpt; ii++){
                                                list_mofif_mmap.push_back({list_update[i][j-cpt+ii].second,new_id}); // TO DO ID COLOR
                                            }
                                            ...
                                            for(uint i =0; i<cpt;i++){
                                                #pragma omp critical
                                                {
                                                decremente_color(colormap[i], icolor);
                                                }
                                            }
                                            #pragma omp atomic
                                            current_id_color++;
                                        ...
                                        previous_color = colormap[i][icolor];
                                    }
                                    else{
                                        cpt++;
                                    }
                                ...
                                if(cpt == previous_color.get_nb_occ()){
                                    ...
                                    colormap[i][new_id] = color_to_insert; // TO DO ICOLOR UINT64?
                                    for(uint ii = 0; ii<cpt; ii++){
                                        list_mofif_mmap.push_back({list_update[i][list_update[i].size()-cpt+ii].second,new_id}); // TO DO ID COLOR
                                    }
                                    color_to_insert.set_nb_occ(cpt);
...
//                             multi_end = ligne_size;
//                         }
//                         vector<mmer> local_minimizer_list = ml.get_minimizer_list(ligne,multi_begin,multi_end);
//                         #pragma omp critical (ml)
//                         {
...
//                                     bool seen(it != map_current_read_color[idmutex].end());
//                                     if (seen) {
...
//                                             omp_set_lock(&(color_map_mutex[idmutex]));
//                                             colormap[idmutex][previous_icolor].add_idread(global_num_read-1);
//                                             omp_unset_lock(&(color_map_mutex[idmutex]));
//                                         }                                        
//                                         else{
//                                             color_to_verify = Color(previous_color, global_num_read - 1);
//                                             #pragma omp critical(idc)
//                                             {
