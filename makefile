...
LARGE_TO_INDEX = log/celill1M_noN.fa log/bench_coverage_ecoli/ecoli01p_10000_0.01_500.fa log/bench_coverage_cel/cel_10000_0.01_70.fa
...
test_all_index_mono:
	./k2r_index -r $(FILE_TO_INDEX) -b example/output/output_binary -s 32 --min-ab 1 --max-ab 50000 -t 1
	head -n 2 $(FILE_TO_INDEX) | sed -n '1p; 2s/\(.\{40\}\).*/\1/p' > example/sequences/sequence1.fasta
	head -n 4 $(FILE_TO_INDEX) | tail -n 2 | sed -n '1p; 2s/\(.\{40\}\).*/\1/p' > example/sequences/sequence2.fasta
	./k2r_query -f example/sequences/fof.txt -o out_test -b example/output/output_binary -r 1 -t 1 --reads
	bash check.sh fof $(FILE_TO_INDEX) example/sequences/fof.txt out_test

test_all_index_mono_loop:
	for dataset in $(FILE_TO_INDEX); do \
